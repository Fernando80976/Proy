...
const QuestsPanel = () => {
  ...
  const claimMutation = useMutation({
    ...
    onError: (error) => {
      const msg = axios.isAxiosError(error) ? error.response?.data?.detail : "Unknown System Error";
  ...
  if (isLoading) return (
    <div className="flex flex-col items-center justify-center py-32 gap-6 bg-black/20 backdrop-blur-md rounded-3xl border border-white/5">
      <div className="relative">
        <Loader2 className="w-16 h-16 text-system-glow animate-spin opacity-20" />
        <Loader2 className="w-16 h-16 text-system-glow animate-spin absolute inset-0 [animation-duration:3s] blur-sm" />
      </div>
      <p className="text-system-glow font-mono animate-pulse tracking-[0.4em] uppercase text-sm">Synchronizing Quest Log...</p>
    </div>
  );
      ...
      <div className="lg:col-span-7">
        {selectedQuest && selectedQuest.status !== 'claimed' ? (
          <div className="system-panel rounded-2xl p-10 border border-system-glow/20 bg-gradient-to-br from-system-glow/[0.03] to-transparent h-full flex flex-col animate-slide-in relative overflow-hidden">
            ...
            {/* Decoración de fondo para el detalle */}
            ...
            <div className="space-y-10 flex-1 relative z-10">
              <div className="p-8 bg-white/[0.03] rounded-2xl border border-white/5 backdrop-blur-sm">
                ...
                <div className="relative h-4 bg-black/60 rounded-full overflow-hidden p-[2px] border border-white/5">
                  <div
            ...
            {/* BOTÓN RECLAMAR - Únicamente si está completada */}
